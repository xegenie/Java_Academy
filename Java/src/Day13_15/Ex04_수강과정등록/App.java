...
public class App {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 5: no = 0;
				break;
			default: System.out.println("유효한 메뉴 번호를 입력해 주세요.");
				break;
			}
	...
	public static void addStudent() {
		...
		for (int i = 0; i < courseList.size(); i++) {
			...
			if ( course.getName().equals(courseName)) {	// && course.getType ,Getter 만들면 될거같은데
				selectedCourse = course;	// 지정된 과정
				// 등록할 과정은 직장인이라 Course<Worker>로 되어 있고
				// List<Worker> students로 지정된다.
				// 따라서 이 수강생 목록에 추가할 Worker 객체를 생성한다.
				switch (type) {
				case "일반인": newStudent = new Person(name);
					break;
				case "직장인": newStudent = new Worker(name);
					break;
				case "학생": newStudent = new Student(name);
					break;
				}
		...
		// ** 여기서 과정별로 수강할 수 없는 수강생 등록이 이루어지면 예외가 발생하도록 코드 수정
