-- Active: 1725516659181@@127.0.0.1@1521@orcl@JOEUN
...
INSERT INTO MS_FILE 
VALUES (2,1,'Main.java','123','2024/09/05','2024/09/05','gif');
...
-- MS_FILE에 BOARD_NO 추가, MS_BOARD의 BOARD_NO 외래키 지정 연쇄삭제
...
-- MS_REPLY에 BOARD_NO 추가, MS_BOARD의 BOARD_NO 외래키 지정 연쇄삭제
...
FROM EMPLOYEE e, DEPARTMENT d
WHERE e.DEPT_CODE = d.DEPT_ID
    AND e.SALARY > ALL (               -- 모든 D1사원의 급여보다 높은 경우
                    SELECT SALARY
                    FROM EMPLOYEE
                    WHERE DEPT_CODE = 'D1'
                    );
...
SELECT ROWNUM 순번,
     ...
     TRUNC(MONTHS_BETWEEN(SYSDATE, E.HIRE_DATE)/12) 근속연수,
      RPAD(SUBSTR(E.EMP_NO,1,8),14,'*') 주민등록번호,
      TO_CHAR(HIRE_DATE,'YYYY.MM.DD') 입사일자,
      -- 연봉 = (급여 + (급여*보너스)) * 12
     TO_CHAR(( SALARY + ( SALARY * NVL(BONUS, 0) ))*12, '999,999,999')
FROM EMPLOYEE E
     LEFT JOIN DEPARTMENT D ON E.DEPT_CODE = D.DEPT_ID
     LEFT JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
     ;
...
SELECT department_id
    ...
    , LAST_VALUE(salary) 
        OVER (PARTITION BY department_id 
              ...
              ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) AS last_salary
FROM employees;
...
FROM (
    ...
    FROM board
    WHERE ROWNUM <= 50  -- 원하는 페이지 크기를 여기에 지정합니다.
)
WHERE row_num >= 41; -- 원하는 페이지 번호에 맞게 시작하는 행 번호를 여기에 지정합니다.
...
-- 실행결과 출력하도록 설정(SQL PLUS, SQL Developer에서 실행 가능)
SET SERVEROUTPUT ON; 


DECLARE
    VI_NUM      NUMBER;     -- 선언부 (NUMBER 타입 변수)
BEGIN    
    VI_NUM := 100;          -- 실행부
...
BEGIN
    FOR i IN 1..20 LOOP
        CONTINUE WHEN MOD(i, 2) = 0;    -- 짝수일 때, 건너 뜀
...
-- SELECT 문에서 함수 실행
SELECT EMP_NAME, get_dept_title(DEPT_CODE)
FROM EMPLOYEE;
-- PL/SQL 블록에서 함수 실행
DECLARE
...
-- 함수 생성
-- emp_id 를 인자로 넘겨주면
-- 사원 구분을 '매니저', '사원'으로 변환하는 함수를 정의해 보세요.
-- 함수명 : emp_type()
...
END;
/
SELECT EMP_NAME, dept_title(EMP_ID)
FROM EMPLOYEE;
...
-- 직무 변경에 따른 다른 직무 이력 갱신 프로세스를 프로시저로 정의하시오.
-- 1. EMPLYEES 테이블에서 JOB_ID 가 변경
...
/
-- 프로시저 실행
-- 200번 사원의 직무를 IT-PROG 로 변경하고, 직무이력 해당 기간으로 갱신하시오.
EXECUTE PRO_APP_EMP('200','IT PROG','2025/03/02','2027/03/02');
...
-- 115. 사원 번호를 입력하면, 해당 사원을 아래 <조건>에 따라 
-- 승진시키는 프로시저를 완성하시오.
