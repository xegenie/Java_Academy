-- Active: 1725502747953@@127.0.0.1@1521@orcl@ALOHA
-- 1. 시스템 계정 접속 
conn system/123456;

-- 2. ALL_USERS에서 HR계정 조회
...
-- SALLARY를 기준으로 내림차순 정렬하고,
-- FIST NAME을 기준으로 오름차순 정렬하여 조회하는 SQL문을 작성하시오.
SELECT *
FROM employees
...
-- 10. 테이블 EMPLOYEES 의 JOB_ID가 ‘FI_ACCOUNT’ 이거나 ‘IT_PROG’ 인 
-- 사원의 모든 컬럼을 조회하는 SQL 문을 작성하시오.
-- < IN > 사용
SELECT *
FROM employees
...
-- 13. 테이블 EMPLOYEES 의 FIRST_NAME 이 ‘S’로 시작하는 사원의  
...
-- 17. 테이블 EMPLOYEES 의 COMMISSION_PCT가 NULL 인 
-- 사원의 모든 컬럼을 조회하는 SQL 문을 작성하시오.
-- < IS NULL >
SELECT *
FROM employees
...
-- 19. 테이블 EMPLOYEES 의 사원의 HIRE_DATE가 04년 이상인 
-- 모든 컬럼을 조회하는 SQL 문을 작성하시오.
...
-- 20. 테이블 EMPLOYEES 의 사원의 HIRE_DATE가 04년도부터 05년도인 
-- 모든 컬럼을 조회하는 SQL 문을 작성하시오.
SELECT *
FROM employees
WHERE hire_date >= '04/01/01'
...
-- 32.
-- 문자열 부분 출력하기
-- SUBSTR(문자열, 시작번호, 글자수) / SUBSTRB : 바이트 단위
-- 'www.alohaclass.com'
...
SELECT SUBSTRB('www.알로하클래스.com', 1, 3) AS "1"
      ,SUBSTRB('www.알로하클래스.com', 5, 18) AS "2" -- 3byte * 6글자 = 18
...
--  : 해당 문자열에서 1번 위치부터 찾아서 2번째 나오는 A의 위치를 반환
SELECT INSTR('ALOHACAMPUS', 'A', 1) AS "1번째 A"
      ,INSTR('ALOHACAMPUS', 'A', 1, 2) AS "2번째 A"
      ,INSTR('ALOHACAMPUS', 'A', 1, 3) AS "3번째 A"
      ,INSTR('ALOHACAMPUS', 'A', 1, 4) AS "4번째 A"
FROM dual;
...
SELECT FIRST_NAME
    ...
    ,TRUNC(MONTHS_BETWEEN(sysdate, hire_date),0) || '개월' 근무달수
...
SELECT NEXT_DAY( sysdate, 2 ) "다음 월요일"
...
-- < DISTINCT > : 중복없이 조회
-- < NVL(값, 대체할 값) > : 해당 값이 NULL 이면 지정된 값으로 변환
...
SELECT first_name 이름
    , CASE 
      ...
      WHEN DEPARTMENT_ID = 1000 THEN  'Finance'
      ELSE  '부서 없음'
    END 부서
FROM EMPLOYEES
;
...
-- 기본키 제약조건 별도로 지정
-- ,CONSTTRAINT MS_STUDENT_PK     PRIMARY KEY(ST_NO) ENABLE
);

-- UQ(고유키) 추가
ALTER TABLE MS_STUDENT ADD CONSTRAINT MS_STUDENT_UK1 UNIQUE (EMAIL) ENABLE;
ALTER TABLE MS_STUDENT ADD CONSTRAINT MS_STUDENT_PK PRIMARY KEY(ST_NO) ENABLE;
...
-- '2024/03/01'   : 문자 타입이 DATE로 내부적으로 변환되어서 데이터 추가
-- DB 툴을 이용하여 INSERT 한다면, COMMIT을 실행해야 LOCK이 걸리지 않고 적용됨.
COMMIT;
...
SET   GENDER = '??'
WHERE NAME = '윤도현';
-- Error: ORA-02290: 체크 제약조건이 위배되었습니다
-- * 도메인 무결성 보장
